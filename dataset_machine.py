...
#         resized_im = cv2.resize(im,(640,360))
        ...
        if cv2.waitKey(1) & button_state == 1: # GPIO.HIGH
            ...
            csv_dataset_path = os.path.join(saving_dir, f"{dataset_name}-{current_menu_name}.csv")
            
            if current_radio_name not in counter:
                counter[current_radio_name] = 1
            filename = f"{dmy}-{current_menu_name}-{counter[current_radio_name]}.jpg"
            counter[current_radio_name] += 1
            sample = {
                'filename': filename,
            ...
            for index, dataset in enumerate(datasets):
                ...
                for menu in dataset["ingrs"]:
                    ...
                    if ingr_name == "":
                        continue
                    total_weight = int(menu["total_weight_txt"].get()) if menu["total_weight_txt"].get() != "" else 0
                    current_weight = int(menu["current_weight_txt"].get()) if menu["current_weight_txt"].get() != "" else 0
...
#             cv2.resize(im,(640,480))
            im_saving_dir = os.path.join(saving_dir, f"{dataset_name}-{current_menu_name}")
...
def stop_recording():
    msg_box = messagebox.askquestion('Exit Application','Are you sure you want to exit the application',icon = 'warning', default='no')
    if msg_box == 'yes':
        exit_program()
...
def main():
    SAVING_DIR = "/home/pi/Downloads/HX711-master/HX711_Python3/hx711py/datasets/"
...
#         raise OSError("Dangerous: disk may have not enough spaces")
    ...
    for dataset_index in range(NUM_DATASET):
        ...
        dataset["radio"] = tkinter.Radiobutton(root, text=radio_name, padx=20, variable=menu_choice, value=dataset_index)
        ...
        for ingr_index in range(1, 4+1):
            ...
            ingr["total_weight_txt"] = tkinter.Entry(root, width=8, validate="key")
            ingr["total_weight_txt"]['validatecommand'] = (ingr["total_weight_txt"].register(testVal),'%P','%d')
            ingr["current_weight_txt"] = tkinter.Entry(root, width=8, validate="key")
            ingr["current_weight_txt"]['validatecommand'] = (ingr["current_weight_txt"].register(testVal),'%P','%d')
...
#     capturebutton.grid(row=1)
    ...
    for column_index, dataset in enumerate(datasets):
        row = 3
        dataset["radio"].grid(row=row, column=0 + (column_index*SET_COLUMN_WIDTH), columnspan=SET_COLUMN_WIDTH-1)
        row += 1
        dataset["name_lbl"].grid(row=row, column=0 + (column_index*SET_COLUMN_WIDTH))
        dataset["name_txt"].grid(row=row, column=1 + (column_index*SET_COLUMN_WIDTH), columnspan=3)
        row += 1
        tkinter.Label(root, text="Name").grid(row=row, column=1 + (column_index*SET_COLUMN_WIDTH))
        tkinter.Label(root, text="Total").grid(row=row, column=2 + (column_index*SET_COLUMN_WIDTH))
        tkinter.Label(root, text="Current").grid(row=row, column=3 + (column_index*SET_COLUMN_WIDTH))
        row += 1
        for ingr in dataset["ingrs"]:
            ingr["name_lbl"].grid(row=row, column=0 + (column_index*SET_COLUMN_WIDTH))
            ingr["name_txt"].grid(row=row, column=1 + (column_index*SET_COLUMN_WIDTH))
            ingr["total_weight_txt"].grid(row=row, column=2 + (column_index*SET_COLUMN_WIDTH))
            ingr["current_weight_txt"].grid(row=row, column=3 + (column_index*SET_COLUMN_WIDTH))
            row += 1
        ttk.Separator(root, orient='vertical').grid(column=4 + (column_index*SET_COLUMN_WIDTH), row=1, rowspan=9, sticky='ns')
